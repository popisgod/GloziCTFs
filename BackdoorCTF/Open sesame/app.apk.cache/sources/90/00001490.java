...
public class MathKt__MathJVMKt extends MathH {
    ...
    public static final double asinh(double d) {
        if (d < MathJVM.taylor_n_bound) {
            return d <= (-MathJVM.taylor_n_bound) ? -MathKt.asinh(-d) : Math.abs(d) >= MathJVM.taylor_2_bound ? d - (((d * d) * d) / 6) : d;
        } else if (d > MathJVM.upper_taylor_n_bound) {
            if (d > MathJVM.upper_taylor_2_bound) {
                return Math.log(d) + MathJVM.LN2;
            }
            double d2 = d * 2;
    ...
    public static final double acosh(double d) {
        ...
        double d2 = 1;
        double d3 = d - d2;
        if (d3 >= MathJVM.taylor_n_bound) {
            return Math.log(d + Math.sqrt((d * d) - d2));
        }
        double sqrt = Math.sqrt(d3);
        if (sqrt >= MathJVM.taylor_2_bound) {
    ...
    public static final double atanh(double d) {
        if (Math.abs(d) < MathJVM.taylor_n_bound) {
            return Math.abs(d) > MathJVM.taylor_2_bound ? d + (((d * d) * d) / 3) : d;
        }
        double d2 = 1;
    ...
    /* renamed from: ln */
    private static final double m3ln(double d) {
        return Math.log(d);
    }
    ...
    /* renamed from: ln */
    private static final float m2ln(float f) {
