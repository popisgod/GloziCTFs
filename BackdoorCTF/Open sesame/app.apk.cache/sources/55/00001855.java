...
public abstract class ChannelFlow<T> implements FusibleFlow<T> {
        ...
        if (bufferOverflow == BufferOverflow.SUSPEND) {
            ...
            if (i2 != -3) {
                if (i != -3) {
                    if (i2 != -2) {
                        if (i != -2) {
                            if (Debug.getASSERTIONS_ENABLED() && this.capacity < 0) {
                                throw new AssertionError();
                            }
                            if (Debug.getASSERTIONS_ENABLED() && i < 0) {
                                throw new AssertionError();
                            }
                            i2 = this.capacity + i;
                            if (i2 < 0) {
    ...
    public ReceiveChannel<T> produceImpl(CoroutineScope coroutineScope) {
        return Produce.produce$default(coroutineScope, this.context, getProduceCapacity$kotlinx_coroutines_core(), this.onBufferOverflow, CoroutineStart.ATOMIC, null, getCollectToFun$kotlinx_coroutines_core(), 16, null);
    }
