...
public class ViewTransition {
    ...
    private void parseViewTransitionTags(Context context, XmlPullParser parser) {
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(parser), C0177R.styleable.ViewTransition);
        int indexCount = obtainStyledAttributes.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int index = obtainStyledAttributes.getIndex(i);
            if (index == C0177R.styleable.ViewTransition_android_id) {
                this.mId = obtainStyledAttributes.getResourceId(index, this.mId);
            } else if (index == C0177R.styleable.ViewTransition_motionTarget) {
            ...
            } else if (index == C0177R.styleable.ViewTransition_onStateTransition) {
                this.mOnStateTransition = obtainStyledAttributes.getInt(index, this.mOnStateTransition);
            } else if (index == C0177R.styleable.ViewTransition_transitionDisable) {
                this.mDisabled = obtainStyledAttributes.getBoolean(index, this.mDisabled);
            } else if (index == C0177R.styleable.ViewTransition_pathMotionArc) {
                this.mPathMotionArc = obtainStyledAttributes.getInt(index, this.mPathMotionArc);
            } else if (index == C0177R.styleable.ViewTransition_duration) {
                this.mDuration = obtainStyledAttributes.getInt(index, this.mDuration);
            } else if (index == C0177R.styleable.ViewTransition_upDuration) {
                this.mUpDuration = obtainStyledAttributes.getInt(index, this.mUpDuration);
            } else if (index == C0177R.styleable.ViewTransition_viewTransitionMode) {
                this.mViewTransitionMode = obtainStyledAttributes.getInt(index, this.mViewTransitionMode);
            } else if (index == C0177R.styleable.ViewTransition_motionInterpolator) {
    ...
    public void applyTransition(ViewTransitionController controller, MotionLayout layout, int fromId, ConstraintSet current, final View... views) {
        ...
        MotionScene.Transition transition = new MotionScene.Transition(-1, layout.mScene, C0177R.C0180id.view_transition, fromId);
        ...
        layout.transitionToEnd(new Runnable() { // from class: androidx.constraintlayout.motion.widget.ViewTransition$$ExternalSyntheticLambda0
            ...
            public final void run() {
                ViewTransition.this.m143x14d7500(views);
            }
        });
    }
    ...
    public /* synthetic */ void m143x14d7500(View[] viewArr) {
        if (this.mSetsTag != -1) {
    ...
    private void updateTransition(MotionScene.Transition transition, View view) {
        ...
        if (keyFrames != null) {
            ...
            while (it.hasNext()) {
                keyFrames2.addKey(it.next().mo1762clone().setViewId(id));
            }
