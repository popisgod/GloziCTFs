...
public final class WorkQueue {
    ...
    public final long tryStealFrom(WorkQueue workQueue) {
        if (!Debug.getASSERTIONS_ENABLED() || getBufferSize$kotlinx_coroutines_core() == 0) {
            Task pollBuffer = workQueue.pollBuffer();
            if (pollBuffer != null) {
                Task add$default = add$default(this, pollBuffer, false, 2, null);
                if (!Debug.getASSERTIONS_ENABLED() || add$default == null) {
                    return -1L;
                }
    ...
    public final long tryStealBlockingFrom(WorkQueue workQueue) {
        if (!Debug.getASSERTIONS_ENABLED() || getBufferSize$kotlinx_coroutines_core() == 0) {
            ...
            for (int i2 = workQueue.consumerIndex; i2 != i; i2++) {
                ...
                if (task != null && task.taskContext.getTaskMode() == 1 && ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m1m(atomicReferenceArray, i3, task, null)) {
    ...
    private final long tryStealLastScheduled(WorkQueue workQueue, boolean z) {
        ...
        } while (!C0133xc40028dd.m146m(lastScheduledTask$FU, workQueue, task, null));
        add$default(this, task, false, 2, null);
        return -1L;
    }
    ...
    private final void decrementIfBlocking(Task task) {
        ...
        if (Debug.getASSERTIONS_ENABLED() && decrementAndGet < 0) {
            throw new AssertionError();
        }
    }
}