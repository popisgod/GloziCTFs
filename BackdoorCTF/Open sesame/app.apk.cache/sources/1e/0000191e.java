...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedList.CLOSED) {
                        m1714constructorimpl = SegmentOrClosed.m1714constructorimpl(ConcurrentLinkedList.CLOSED);
                        break;
                    }
                    ...
                    m1714constructorimpl = SegmentOrClosed.m1714constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1719isClosedimpl(m1714constructorimpl)) {
                Segment m1717getSegmentimpl = SegmentOrClosed.m1717getSegmentimpl(m1714constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m1717getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m1717getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (C0133xc40028dd.m146m(tail$FU, this, segment, m1717getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m1717getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1717getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1717getSegmentimpl(m1714constructorimpl);
        i2 = SemaphoreKt.SEGMENT_SIZE;
        int i3 = (int) (andIncrement % i2);
        if (ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m1m(semaphoreSegment4.acquirers, i3, null, cancellableContinuation)) {
        ...
        if (ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m1m(semaphoreSegment4.acquirers, i3, symbol, symbol2)) {
            cancellableContinuation.resume(Unit.INSTANCE, this.onCancellationRelease);
            return true;
        } else if (Debug.getASSERTIONS_ENABLED()) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (segment.getId() >= m1717getSegmentimpl.getId()) {
                    ...
                    break;
                } else if (C0133xc40028dd.m146m(head$FU, this, segment, m1717getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1717getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m1m(semaphoreSegment4.acquirers, i4, symbol3, symbol4);
        }
