...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f102mX = new float[min];
        this.f103mY = new float[min];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < min; i2++) {
            pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
            this.f102mX[i2] = fArr[0];
            this.f103mY[i2] = fArr[1];
        }
        if (Math.abs(this.f102mX[0]) <= 1.0E-5d && Math.abs(this.f103mY[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f102mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f103mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < min) {
                    float[] fArr2 = this.f102mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        int i6 = min - 1;
        throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f102mX[0] + "," + this.f103mY[0] + " end:" + this.f102mX[i6] + "," + this.f103mY[i6]);
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f102mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f102mX[i2]) {
